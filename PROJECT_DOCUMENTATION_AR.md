...
## إصدار 25 أغسطس 2026 — Accounting Reliability + Accordion Navigation + Arabic UI Audit

### ACCOUNTING

تمت مراجعة مسار البيانات كاملًا من `LocalStorageErpRepository` إلى التطبيع والخدمات المحاسبية ثم Zustand والواجهات. لم يُعَد تنفيذ إصلاح تصنيف القوائم السابق، بل بُني عليه. الأسباب الجذرية الجديدة التي عولجت:

- كان الرصيد الجاري في دفتر الأستاذ يجمع كل الحسابات في سلسلة واحدة؛ أصبح لكل حساب رصيد مستقل مع إدخال الحركة السابقة كبداية عند تحديد تاريخ `من`.
- مرشح المشروع كان يعتمد أحيانًا على `entry.projectId` فقط ويتجاهل البعد الموجود على سطر القيد؛ أصبح يقبل البعد على القيد أو السطر دون تسريب مشروع آخر.
- لوحة الرقابة كانت تحسب مستندات ومشروعات وقيودًا وحسابات من شركات أخرى؛ أصبحت كل مؤشرات الاستثناءات والتسوية والربط معزولة حسب الشركة الحالية.
- اختيار اسم الحساب في القيد اليدوي كان يمكن أن يلتقط حسابًا من شركة أخرى عند تكرار الكود؛ أصبح البحث مقيدًا بالشركة.
- الشركة الجديدة ذات دليل الحسابات الفارغ كانت تدخل حلقة Empty State لا تسمح بإنشاء الحساب؛ أضيف نموذج حقيقي لإضافة الحساب مع النوع والحساب الأب والتصنيف المالي التلقائي القابل للتعديل.
- أضيفت دوال `safeNumber / safeSum / safeDivide`، ووسع تطبيع البيانات المحلية للأوامر والمخزون والمصروفات والمستخلصات وسطـورها، بحيث لا تصل `NaN` أو مصفوفة تالفة إلى واجهات الحسابات.

دفتر الأستاذ يدعم الآن مرشحات الحساب والمشروع ومن/إلى، ويعرض حركة المدين والدائن والفرق، مع رصيد جارٍ صحيح لكل حساب. جميع القيود والمرشحات ما زالت تعمل داخل المتصفح فقط.

### SIDEBAR

استُبدلت القائمة المسطحة بمكوّن `SidebarNavigation` مبني على تعريف بيانات مركزي:

- 55 Module فعلية فقط، دون إضافة Route وهمي.
- رابط مباشر للوحة الإدارة، و9 مجموعات رئيسية قابلة للفتح، مع 5 مجموعات فرعية متداخلة داخل الحسابات.
- `aria-expanded` و`aria-controls` و`aria-current` ودعم لوحة المفاتيح وحالات Focus واضحة.
- فتح تلقائي لأب وأب-الأب للمسار النشط، وله الأولوية على الحالة المحفوظة.
- حفظ اللغة والمسار والمجموعات المفتوحة عبر `UiPreferencesRepository` بالمفتاح `sitecost-ui-preferences-v1`؛ لا يصل Component مباشرة إلى `localStorage`.
- Drawer موبايل يغلق بعد اختيار الوحدة، وRTL/LTR يعملان دون قص أفقي، مع Scroll داخلي خفيف وبقاء Header/Footer ثابتين.

### ARABIC LOCALIZATION

- وُحّدت مصطلحات القائمة: `دفتر الأستاذ العام`، `قائمة المركز المالي`، `قائمة التغيرات في حقوق الملكية`، `الربط المحاسبي`، و`إقفال الفترة`.
- احتُفظ بالاختصارات الفنية المقصودة داخل صياغة عربية: WBS وBOQ وRFI وNCR وVAT وSubmittals وTransmittals.
- نُظفت أكثر من 80 موضع عرض متداخل عربي/إنجليزي في رأس الصفحة، القيد متعدد السطور، الربط والسياسات المحاسبية، تسوية تكلفة المشروع، الإقفال، المستندات والمرشحات والحالات الأساسية.
- القيم الداخلية مثل `draft` و`reviewed` و`posted` وأنواع المستندات لم تتغير؛ التعديل في طبقة العرض فقط.
- أصبح Accounting Error Boundary ثنائي اللغة، ولا يعرض Stack Trace للمستخدم في Production.

### TESTS

نتائج القبول المحلية النهائية:

| البوابة | النتيجة |
| --- | --- |
| TypeScript `tsc --noEmit` | ✅ بلا أخطاء |
| ESLint `--max-warnings=0` | ✅ بلا أخطاء أو تحذيرات |
| Vitest | ✅ 9 ملفات، 62/62 اختبارًا |
| Next.js Production Build | ✅ Static output؛ `/` وmanifest و404 فقط |
| Playwright Accounting | ✅ 4/4 Desktop + Mobile |
| Playwright Full Regression | ✅ 6/6 Desktop + Mobile |

تغطي الاختبارات Fresh Company وCurrent Demo وLegacy nested data وMalformed numeric data، وعزل رصيد الأستاذ حسب الحساب والمشروع، وتصنيف القوائم، وفتح Accordion واستعادة المسار بعد Refresh، وإغلاق Drawer، ومسار Tender → Project → Correspondence.

### GIT

- Functional commit: `423cb83` — `feat: harden accounting and add accordion navigation`.
- الفرع: `main`.
- لا توجد تغييرات غير متعلقة بالمهمة أزيلت أو استُبدلت.
...
المسار المعتمد ما زال:

```text
React Components
→ Application Actions / Zustand UI State
→ Business Services
→ Repository Interfaces
→ LocalStorage / SessionStorage
```

فحص المصدر لم يجد `app/api` أو `route.ts` أو `use server` أو Prisma/Supabase/Firebase/Express أو اتصال قاعدة بيانات. البناء النهائي يخرج صفحات Static فقط. المصادقة ما زالت PBKDF2 + SHA-256 + Salt وسجلات مستخدم محلية وSessionStorage.

### DOCUMENTATION

هذا القسم هو سجل الإصدار الرئيسي ويشرح السبب الجذري، الملفات المعمارية الجديدة، سلوك الترحيل، مصفوفة الاختبارات، وحالة Git/Vercel دون إخفاء القيود.

### KNOWN LIMITATIONS

- البيانات مرتبطة بالمتصفح والجهاز الحاليين؛ نقلها يتم عبر Backup/Restore JSON يدويًا.
- لا يوجد تزامن متعدد المستخدمين أو صلاحيات Production Server؛ المصادقة الحالية نموذج محلي مقصود.
- المرفقات تحفظ Metadata فقط، ولا تُرفع ملفات فعلية إلى خدمة خارجية.
- السجلات القديمة التي تشير إلى كود حساب غير موجود لا تُخترع لها حسابات تلقائيًا؛ تظهر Warning ويصححها المستخدم من دليل الحسابات أو القيد.
- قواعد الضرائب والإقفال والسياسات قابلة للضبط كنموذج ERP محلي، وتحتاج اعتماد محاسب قانوني قبل استخدام إنتاجي حقيقي.
- الأكواد وأسماء المستخدم والمراجع والمصطلحات الفنية المقصودة لا تُترجم حتى لا تتلف البيانات.

---

آخر تحديث: **25 أغسطس 2026 — Accounting Reliability, Accordion Navigation & Arabic UI Audit**.
