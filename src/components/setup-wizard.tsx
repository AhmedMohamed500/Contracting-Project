  ...
  return <div className="auth-screen setup-auth" dir={language === "ar" ? "rtl" : "ltr"}>
    <section className="auth-panel setup-panel">
      <div className="auth-card setup-card">
        <div className="setup-card-tools"><span>{t("إعداد مساحة شركتك", "Configure your workspace")}</span><button className="setup-language" onClick={() => setLanguage(language === "ar" ? "en" : "ar")}><Languages size={15}/>{language === "ar" ? "EN" : "AR"}</button></div>
        <div className="setup-brand"><CompanyLogo company={values}/><div><h1>{branding.productName}</h1><p>{t(branding.taglineArabic, branding.tagline)}</p></div></div>
        ...
        <div className="setup-stepper" aria-label={t("خطوات إعداد النظام", "System setup steps")}>{steps.map((label, index) => <div className={`${index < step ? "complete" : ""} ${index === step ? "current" : ""}`} key={label}><i>{index < step ? <CheckCircle2 size={15}/> : index + 1}</i><span>{label}</span></div>)}</div>
        <div className="setup-mobile-step"><div><span>{t(`الخطوة ${step + 1} من 5`, `Step ${step + 1} of 5`)}</span><strong>{steps[step]}</strong></div><progress max="5" value={step + 1}/></div>
        ...
        {step === 3 && <div className="form-grid administrator-grid"><div className="setup-icon-heading"><UserRound/><div><h2>{t("إنشاء مدير النظام", "Create administrator")}</h2><p>{t("أنشئ حساب المدير الأول للنظام. سيتم حفظ كلمة المرور بشكل مشفر محليًا في النسخة التجريبية.", "Create the first system administrator. The password will be stored locally as a secure hash in this prototype.")}</p></div></div><SetupField label={t("الاسم الكامل", "Full name")}><input className="input" autoComplete="name" value={values.adminFullName} onChange={(e) => set("adminFullName", e.target.value)}/></SetupField><SetupField label={t("اسم المستخدم", "Username")}><input className="input" dir="ltr" autoComplete="username" value={values.username} onChange={(e) => set("username", e.target.value)}/></SetupField><SetupField label={t("كلمة المرور", "Password")}><div className="password-field"><input className="input" dir="ltr" type={showPassword ? "text" : "password"} autoComplete="new-password" value={values.password} onChange={(e) => set("password", e.target.value)}/><button type="button" onClick={() => setShowPassword(!showPassword)}>{showPassword ? <EyeOff/> : <Eye/>}</button></div><small className="password-rule">{t("8 أحرف على الأقل، وتتضمن حرفًا ورقمًا.", "At least 8 characters, including a letter and a number.")}</small></SetupField><SetupField label={t("تأكيد كلمة المرور", "Confirm password")}><input className="input" dir="ltr" type="password" autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)}/></SetupField></div>}
...
export function ConstructionVisual({ t }: { t: (ar: string, en: string) => string }) { return <section className="construction-visual" aria-label={t("موقع إنشاءات حديث لمشروع تجاري", "Modern commercial construction site")}><Image priority fill sizes="(max-width: 760px) 100vw, 62vw" className="construction-photo" src="/sitecost-construction-visual.webp" alt={t("هيكل خرساني وفولاذي حديث تحت الإنشاء مع رافعة برجية ومخططات هندسية", "Modern concrete and steel structure under construction with a tower crane and engineering plans")}/><div className="construction-photo-treatment"/><div className="visual-message"><HardHat/><div><strong>{t("من الموقع إلى القوائم المالية", "From site operations to financial statements")}</strong><span>{t("إدارة هندسية ومالية مترابطة لمشروعات المقاولات", "Connected engineering and financial control for construction projects")}</span></div></div></section>; }
